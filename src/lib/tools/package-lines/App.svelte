<script lang="ts">
  ...
  import { runPackageLines, type TargetSchema } from "./pipeline/index";
  ...
  async function handleRun(): Promise<void> {
    ...
    try {
      const result = await runPackageLines(duckdbState.conn!, schema);
...
<div class="layout">
  <aside class="sidebar">
    <header>
      ...
      <h1>Package Lines</h1>
      <p class="blurb">
        Extract every shared boundary and exterior edge between admin units, classified by the
        coarsest level at which two neighbours diverge.
      </p>
    </header>
