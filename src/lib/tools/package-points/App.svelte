<script lang="ts">
  ...
  async function handleRun(): Promise<void> {
    ...
    try {
      const result = await runPackagePoints(duckdbState.conn!, schema);
      resultGeoJSON = result.resultGeoJSON;
      resultBounds = result.bounds;
      levels = result.levels;
      rootInjected = result.rootInjected;
      ran = true;
    } catch (e) {
...
<div class="layout">
  <aside class="sidebar">
    <header>
      ...
      <h1>Package Points</h1>
      <p class="blurb">
        Produce one representative point per admin unit, at every level a polygon layer's
        hierarchy contains, in a single combined output.
      </p>
    </header>
    ...
    {#if ran}
      <section class="step">
        ...
        <p class="summary-line">
          Level{levels.length === 1 ? "" : "s"} {levels.join(", ")}{rootInjected
            ? " (level 0 auto-detected as a whole-table root)"
            : ""}.
        </p>
      </section>
